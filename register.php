...
if(empty(trim($_POST['fullname']))){
    ...
    $fullname_err = "fullname can only contain letters, numbers, and underscores.";
} else{
 
// Processing form data when form is submitted
if($_SERVER["REQUEST_METHOD"] == "POST"){
    if(empty(trim($_POST["fullname"]))){
        $username_err = "Please enter a fullname.";
    } elseif(!preg_match('/^[a-zA-Z0-9_]+$/', trim($_POST["fullname"]))){
        $fullname_err = "fullname can only contain letters, numbers, and underscores.";
    } else{
        // Prepare a select statement
        $sql = "SELECT id FROM user_list WHERE fullname = ?";
    ...
    if(empty($fullname_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO user_list (fullname, username, password) VALUES (?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ss", $param_fullname, $param_username, $param_password);
